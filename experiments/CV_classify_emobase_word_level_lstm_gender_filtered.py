...
experiment_dir, logger = create_experiment(EXPERIMENTS_FOLDER, class_groups, "CV_classify_emobase_word_level_gender_filtered", use_timestamp=True)
...
for i in range(0, nr_of_folds):
    ...
    means, stddevs = get_means_and_stddevs_from_sequence_dataset(metadata, emobase_features, class_groups, train_folds, take_gender = take_gender)
    ...
    test_dataset = create_sequence_dataset_from_metadata(metadata, emobase_features, class_groups, test_folds, take_gender = take_gender)
