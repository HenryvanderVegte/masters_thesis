...
emobase_features_path = os.path.join(ROOT_FOLDER, 'datasets//IEMOCAP//features//audio//emobase_word_level_50ms_buffer.npy')
...
experiment_dir, logger = create_experiment(EXPERIMENTS_FOLDER, class_groups, "CV_early_fusion_word_level_joined_hidden_activation", use_timestamp=True)
...
for i in range(0, nr_of_folds):
    ...
    emobase_model_path = os.path.join(ROOT_FOLDER, 'models//CV//8//CV_classify_emobase_50ms_buffer//' + str(i) + '//model.pth')
    ...
    os.mkdir(fold_path)
    test_golds, test_preds = train_two_modality_rnn_join_hidden(word_embedding_resources, emobase_resources, id_to_name, fold_path, joined_model, logger, joined_model_params)
